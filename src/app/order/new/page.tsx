...
export default function NewOrderPage() {
  ...
  const discountAmount = hasDiscount ? Math.round((grandTotal * discountPercent) / 100) : 0
            ...
            <div className="rounded-2xl bg-teal-500/10 backdrop-blur-2xl p-5 shadow-lg border border-teal-500/20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                <div className="relative md:col-span-2" onClick={(e) => e.stopPropagation()}>
                  ...
                  <div className="flex gap-2">
                    ...
                    {/* توجه: مسیر زیر با فرض وجود صفحه‌ی فرم مستقیمِ افزودن مشتری در /customers/new تنظیم شده.
                        اگر مسیر واقعی پروژه‌ی شما چیز دیگری است (مثلاً ?new=1) این آدرس را با همان جایگزین کنید. */}
                    <button
                      type="button"
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                <div>
                  ...
                  <select ref={customerGroupRef} value={customerGroup} onChange={(e) => setCustomerGroup(e.target.value)} onKeyDown={(e) => handleEnter(e, priorityRef)} className={selectClass}>
                ...
                <div>
                  ...
                  <select ref={productionLineRef} value={productionLine} onChange={(e) => setProductionLine(e.target.value)} onKeyDown={(e) => handleEnter(e, customerGroupRef)} className={selectClass}>
            ...
            <div className="rounded-2xl bg-teal-500/10 backdrop-blur-2xl p-5 shadow-lg border border-teal-500/20">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-9 gap-2 items-end">
                <div className="lg:col-span-2">
                  ...
                  <select ref={productRef} value={newProduct} onChange={(e) => setNewProduct(e.target.value)} onKeyDown={(e) => handleEnter(e, unitPriceRef)} className={selectClass}>
          ...
          {showInvoice && (
            <div className="fixed inset-0 z-[160] flex items-center justify-center bg-black/60 p-4 print:static print:bg-white print:p-0 print:block">
              <div className="w-full max-w-5xl max-h-[94vh] overflow-y-auto rounded-2xl bg-white shadow-2xl print:max-h-none print:overflow-visible print:shadow-none print:rounded-none print:max-w-none">
                ...
                <div className="sticky top-0 z-10 flex items-center justify-between bg-teal-700 text-white px-5 py-3 rounded-t-2xl print:hidden flex-wrap gap-y-2">
                  ...
                  <div className="flex gap-2 items-center flex-wrap">
                    {/* تخفیف: مثل ارزش افزوده یک چک‌باکس دارد، با یک پاپ‌آپ برای انتخاب درصد ۳ تا ۱۵ */}
                    <div className="relative" onClick={(e) => e.stopPropagation()}>
                      <label className="flex items-center gap-2 text-sm cursor-pointer">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setShowDiscountMenu(e.target.checked)
                          }}
                      ...
                      {showDiscountMenu && (
                        <div className="absolute top-8 left-0 z-50 bg-white rounded-xl shadow-2xl border border-teal-200 p-2 grid grid-cols-3 gap-1 w-44">
                          {discountOptions.map((p) => (
                            <button
                              ...
                              className={`rounded-lg px-2 py-1.5 text-sm font-bold ${
                                discountPercent === p ? "bg-teal-600 text-white" : "bg-teal-50 text-teal-800 hover:bg-teal-100"
                              }`}
                            >
                              {p}٪
                            </button>
                ...
                <div
                  ...
                  ref={invoiceRef}
                  className="p-6 bg-white relative print:p-4 overflow-hidden"
                  dir="rtl"
                  style={{ fontFamily: "Vazirmatn, Tahoma, Arial, sans-serif" }}
                >
                  {/*
                    بک‌گراند تزئینیِ پیش‌فاکتور: فقط اشکال هندسیِ گوشه‌های سربرگ (بدون لوگو/متن/آدرس).
                    این دو فایل باید از خودِ سربرگ (CamScanner) کراپ و به‌صورت PNG با پس‌زمینه‌ی
                    شفاف در public/ پروژه قرار بگیرند؛ چون این کد نمی‌تواند خودش گرافیک را از PDF
                    استخراج کند. تا وقتی این دو فایل ساخته نشوند، این دو تگ img چیزی نمایش نمی‌دهند.
                  */}
                  <img
                    src="/invoice-pattern-top-right.png"
                    alt=""
                    aria-hidden="true"
                    className="pointer-events-none select-none absolute top-0 left-0 w-36 md:w-52 opacity-90"
                    onError={(e) => { (e.target as HTMLImageElement).style.display = "none" }}
                  />
                  <img
                    src="/invoice-pattern-bottom-left.png"
                    alt=""
                    aria-hidden="true"
                    className="pointer-events-none select-none absolute bottom-0 right-0 w-36 md:w-52 opacity-90"
                    onError={(e) => { (e.target as HTMLImageElement).style.display = "none" }}
                  />
                      ...
                      <div className="text-sm text-gray-600 space-y-1">
                        ...
                        {hasDiscount && (
                          <p className="text-rose-700 font-medium">
                            تخفیف ({discountPercent}٪): -{discountAmount.toLocaleString("en-US")} ریال
                          </p>
                        )}
                        {isOfficialInvoice && (
